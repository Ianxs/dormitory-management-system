...
<body class="flex bg-gray-900"
    ...
    <div id="app" class="flex justify-between w-full">
                        ...
                        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                            ...
                            <tbody>
                                @php
                                    ...
                                    $startDate = new DateTime($tenant->created_at);
                                    $endDate = $tenant->lease_end ? new DateTime($tenant->lease_end) : new DateTime(); // now

                                    // Calculate months difference, rounding up partial months to full months
                                    $interval = $startDate->diff($endDate);

                                    $monthsOfStay = $interval->y * 12 + $interval->m;

                                    // If there are any remaining days, count as an additional month
                                    if ($interval->d > 0) {
                                        $monthsOfStay++;
                                    }

                                    // Ensure at least 1 month is counted
                                    $monthsOfStay = max(1, $monthsOfStay);

                                    // Calculate due amount: subscription price * occupants * months of stay
                                    $dueAmount = $subscriptionPrice * $tenant->total_occupants * $monthsOfStay;
        ...
        <script>
            ...
            const hasSuccess = @json(session('success'));
            const hasError = @json(session('error')) || @json($errors->any());

            // Success: Close modal after 2 seconds if Vue app is loaded
            if (hasSuccess && window.app && typeof window.app.isProfileModalOpen !== 'undefined') {
                setTimeout(() => {
                    window.app.isProfileModalOpen = false;
                }, 2000);
            }

            // Error: Open the profile modal if there was an error
            if (hasError && window.app && typeof window.app.isProfileModalOpen !== 'undefined') {
                window.app.isProfileModalOpen = true;
            }
        </script>
